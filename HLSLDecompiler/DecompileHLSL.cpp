...
#undef DT
	...
	char *_convertToInt(char *target, bool fixupCBs)
	{
		...
		{
			size_t size = 0;
			for (pos++; pos[size]; size++)
				if (!strchr("xyzwrgba", pos[size]))
					break;
			if (!pos[size] && size != 1)
	...
	bool translate_uav_typed_var(Shader *shader, const char *c, size_t &pos, size_t &size, Instruction *instr,
			std::string ret[4], bool *combined, char *reg, char *off, Operand *addr, Operand *uav, int swiz_offsets[4])
	{
		Operand dst0 = instr->asOperands[0];
		ResourceGroup group = (ResourceGroup)-1;
		ResourceBinding *bindInfo;
		...
		*combined = false;
		...
		if (off[0] == 'c')
			group = RGROUP_CBUFFER;

		if (GetResourceFromBindingPoint(RGROUP_UAV, uav->ui32RegisterNumber, shader->sInfo, &bindInfo))
		{
			int swiz_offset = 0;
			if (sscanf_s(off, "%d", &swiz_offset) != 1) {
				if (strcmp(strchr(reg, '.'), ".x")) {
			...
			sprintf(buffer, "%s[%s]",
					bindInfo->Name.c_str(), ci(off).c_str());
			// Returning all components combined together:
			*combined = true;
			ret[0] = buffer;
			return true;
	...
	bool translate_raw_var(Shader *shader, const char *c, size_t &pos, size_t &size, Instruction *instr,
			std::string ret[4], bool *combined, char *off, char *reg, Operand *texture, int swiz_offsets[4])
	{
		Operand dst0 = instr->asOperands[0];
		ResourceGroup group = (ResourceGroup)-1;
		ResourceBinding *bindInfo;
		...
		*combined = false;
		...
		if (group != (ResourceGroup)-1 && GetResourceFromBindingPoint(group, texture->ui32RegisterNumber, shader->sInfo, &bindInfo))
		{
			...
			{
				// This StructuredBuffer is using a primitive type rather
				// than a structure (e.g. Buffer<float4> foo).
				DataType struct_type = TranslateType(struct_type_i->second.c_str());
				int swiz_offset = 0;
	...
	void parse_ld_uav_typed(Shader *shader, const char *c, size_t &pos, size_t &size, Instruction *instr)
	{
		std::string translated[4];
		char buffer[512];
		bool combined;

		char *dst = op1, *off = op2, *reg = op3;
		Operand dst0 = instr->asOperands[0];
		Operand adr0 = instr->asOperands[1];
		...
		if (translate_uav_typed_var(shader, c, pos, size, instr, translated, &combined, reg, off, &adr0, &texture, swiz_offsets)) {
			if (combined) {
				sprintf(buffer, "  %s = %s;\n", writeTarget(dst), translated[0].c_str());
				appendOutput(buffer);
			} else {
	...
	void parse_store_uav_typed(Shader *shader, const char *c, size_t &pos, size_t &size, Instruction *instr)
	{
		std::string translated[4];
		char buffer[512];
		bool combined;

		// store_uav_typed u1.x, r0.yyyy, r0.xxxx
		char *dst = op1, *off = op2, *src = op3;
		Operand dst0 = instr->asOperands[0];
		Operand adr0 = instr->asOperands[1];
		Operand src0 = instr->asOperands[2];

		remapTarget(dst);
		remapTarget(off);
		int swiz_offsets[4] = {0, 4, 8, 12};

		if (translate_uav_typed_var(shader, c, pos, size, instr, translated, &combined, dst, off, &adr0, &dst0, swiz_offsets)) {
			if (combined) {
				applySwizzle(dst, src);
				sprintf(buffer, "  %s = %s;\n", translated[0].c_str(), ci(src).c_str());
				appendOutput(buffer);
	...
	void parse_ld_raw(Shader *shader, const char *c, size_t &pos, size_t &size, Instruction *instr)
	{
		std::string translated[4];
		char buffer[512];
		bool combined;
		...
		if (translate_raw_var(shader, c, pos, size, instr, translated, &combined, off, reg, &texture, swiz_offsets)) {
			if (combined) {
				sprintf(buffer, "  %s = %s;\n", writeTarget(dst), translated[0].c_str());
				appendOutput(buffer);
			} else {
	...
	void parse_store_raw(Shader *shader, const char *c, size_t &pos, size_t &size, Instruction *instr)
	{
		std::string translated[4];
		char buffer[512];
		bool combined;

		// store_raw u1.x, l(0), v1.x
		char *dst = op1, *off = op2, *src = op3;
		Operand dst0 = instr->asOperands[0];
		Operand src0 = instr->asOperands[2];
		...
		if (translate_raw_var(shader, c, pos, size, instr, translated, &combined, off, dst, &dst0, swiz_offsets)) {
			if (combined) {
				applySwizzle(dst, src);
				sprintf(buffer, "  %s = %s;\n", translated[0].c_str(), ci(src).c_str());
				appendOutput(buffer);
	...
	void ParseCode(Shader *shader, const char *c, size_t size)
	{
		...
		while (pos < size && iNr < inst_count)
		{
			...
			else if (!strcmp(statement, "dcl_uav_typed_buffer"))
			{
				int bufIndex = 0;
				char type[4][sizeof "float"]{};
				if (sscanf_s(op1, "(%[^,],%[^,],%[^,],%[^)])",
					type[0], sizeof type[0],
					type[1], sizeof type[1],
					type[2], sizeof type[2],
					type[3], sizeof type[3]) != 4)
				{
					logDecompileError("Error parsing uav typed buffer type: " + string(op1));
					return;
				}
				if (sscanf_s(&op2[1], "%d", &bufIndex) != 1)
				{
					logDecompileError("Error parsing uav typed buffer register: " + string(op2));
					return;
				}
				// Similar concern to the constant buffers missing reflection info above - if we don't
				// have the uav definitions we have to manufacture our own. Without type information
				// we assume everything is a float because we can't do any better. This is worse news than
				// a constant buffer missing reflection info as structured buffers tend to contain much
				// more varied data types than constant buffers in practice. Ideally we should move to
				// a model where we do reinterpret casts whenever we need something other than a float.
				if (mUAVNames.empty())
				{
					sprintf(buffer, "RWBuffer<%s> u%d : register(u%d);\n\n",
						type[0], bufIndex, bufIndex);
					mOutput.insert(mOutput.begin(), buffer, buffer + strlen(buffer));
					mCodeStartPos += strlen(buffer);

					if (strcmp(type[0], type[1]) || strcmp(type[0], type[2]) || strcmp(type[0], type[3])) {
						sprintf(buffer, "FIXME: RWBuffer types differ (%s,%s,%s,%s)\n\n",
			...
			else if (!strcmp(statement, "dcl_tgsm_raw"))
			{
				int bufIndex = 0;
				int byteCount = 0;
				if (sscanf_s(op1, "g%d", &bufIndex) != 1)
				{
					logDecompileError("Error parsing tgsm raw buffer register: " + string(op1));
					return;
				}
				if (sscanf_s(op2, "%d", &byteCount) != 1)
				{
					logDecompileError("Error parsing tgsm raw buffer byte count: " + string(op2));
					return;
				}
				sprintf(buffer, "groupshared float g%d[%d];\n",
					bufIndex, byteCount / 4);
				mOutput.insert(mOutput.begin(), buffer, buffer + strlen(buffer));
				mCodeStartPos += strlen(buffer);
			}
			...
			else if (!strcmp(statement, "dcl_thread_group"))
			{
				int x = 0, y = 0, z = 0;
				if (sscanf_s(op1, "%d", &x) != 1)
				{
					logDecompileError("Error parsing thread group x: " + string(op1));
					return;
				}
				if (sscanf_s(op2, "%d", &y) != 1)
				{
					logDecompileError("Error parsing thread group y: " + string(op2));
					return;
				}
				if (sscanf_s(op3, "%d", &z) != 1)
				{
					logDecompileError("Error parsing thread group z: " + string(op3));
					return;
				}
				char *main_ptr = strstr(mOutput.data(), "void main(");
				size_t offset = main_ptr - mOutput.data();
			...
			else
			{
				switch (instr->eOpcode)
				{
					...
					{
						parse_ld_uav_typed(shader, c, pos, size, instr);
						break;
					}

					case OPCODE_STORE_UAV_TYPED:
					{
						parse_store_uav_typed(shader, c, pos, size, instr);
						break;
					}

					case OPCODE_LD_RAW:
					{
						parse_ld_raw(shader, c, pos, size, instr);
						break;
					}

					case OPCODE_STORE_RAW:
					{
						parse_store_raw(shader, c, pos, size, instr);
						break;
					}
